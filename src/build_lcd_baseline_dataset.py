...
MAPPING_FILE = DATA_RAW / "parish_lcd_baseline_station_mapping.csv"
OUTPUT_FILE = DATA_PROCESSED / "lcd_baseline_1981_2010_jul_aug_daily.csv"
...
def reduce_hourly_to_daily(
    ...
    # Keep only baseline years and only July-August
    df = df[
        (df["DATE"] >= pd.Timestamp("1981-07-01"))
        & (df["DATE"] <= pd.Timestamp("2010-08-31 23:59:59"))
...
def main():
    ...
    for _, row in mapping_df.iterrows():
        ...
        print(f"\nBuilding LCD baseline data for {parish} -> {lcd_station_name}")
    ...
    if not reduced_frames:
        print("No LCD hourly-derived baseline data created.")
        return
    ...
    print(f"\nSaved LCD baseline dataset to: {OUTPUT_FILE}")
